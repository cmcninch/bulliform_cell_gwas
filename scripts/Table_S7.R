...
walley_dnsaf <- fread("~/Box Sync/McNinch Projects/Bulliform_Cell_GWAS_Manuscript/data/gene_expression_data/Walley_2016_Protein_dNSAF.csv") %>%
  drop_na() %>%
  dplyr::select(-V2_Gene_Model) %>%
  dplyr::rename("GeneID" = V4_GeneID)

stepflug_fpkm <- fread("~/Box Sync/McNinch Projects/Bulliform_Cell_GWAS_Manuscript/data/gene_expression_data/Stepflug_2016_FPKM_Maize_Atlas_B73_V4.csv") %>%
  filter(Tissue %in% c("Leaf_BottomOfTransistion_V5", "Leaf_TipOfTransistion_V5", 
                       "Leaf_BottomOfTransistion_V7", "Leaf_TipOfTransistion_V7")) %>%
  dplyr::select(GeneID, Tissue, FPKM) %>%
  spread(Tissue, FPKM)
...
candidate_genes <- left_join(candidate_genes, fread("~/Box Sync/McNinch Projects/Bulliform_Cell_GWAS_Manuscript/data/gene_expression_data/GeneID_to_Locus_Lookup.csv"), by = "GeneID") %>%
  ...
  left_join(., walley_dnsaf, by = "GeneID") %>%
  left_join(., stepflug_fpkm, by = "GeneID")

### Write final table ###
##################################################
write_csv(candidate_genes, "~/Desktop/candidate_genes_plus_walley_expression.csv")
