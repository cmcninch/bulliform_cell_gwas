...
bfw_candidate_genes <- filter(candidate_genes, Trait == "BFW") %>%
  group_by(SNP) %>%
  mutate(SNP_Gene_Count = seq(n()))
bfw_candidate_genes$Total_Gene_Count <- 1:nrow(bfw_candidate_genes)

bfw_candidate_gene_breaks <- sort((group_by(bfw_candidate_genes, SNP) %>% dplyr::slice(base::which.max(SNP_Gene_Count)))$Total_Gene_Count)
...
# walley fpkm bff
pheatmap(bfw_candidate_genes[, 8:11],
         cluster_rows = FALSE, 
         cluster_cols = FALSE,
         gaps_row = bfw_candidate_gene_breaks,
         labels_row = bfw_candidate_genes$SNP,
         scale = "row",
         cellwidth = 30,
         cellheight = 20,
         show_rownames = FALSE, 
